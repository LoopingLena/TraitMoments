...
TraitMoments provides two example data frames. The data frame 'communities' contains the relative abundances for 93 species from 20 communities and 'traits' provides information on seven traits for the corresponding species. For some species, no information is available for certain traits, so the data frame 'traits' contains some NAs. Since both data frames are quite large, we just inspect the heads in this example:
